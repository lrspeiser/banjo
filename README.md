...
The [mechanics scorecard](docs/mechanics-scorecard.md) tracks every retained mechanic and platform capability with evidence and next steps. Material-dependent testing retains **glass, oak (wood), and iron** in a growing set. The [trajectory checkpoint](docs/trajectory-checkpoint.md) records 12 passing CTest executables and 30 full-resolution impact runs at five timesteps. Full-node comparisons show that stable bulk rebound can hide unresolved internal motion. Calibrated interfaces, temporal error control and default-runtime integration remain open; the earlier rigid restitution failure remains documented.
