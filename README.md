...
The [mechanics scorecard](docs/mechanics-scorecard.md) tracks every retained mechanic and platform capability with evidence and next steps. Material-dependent testing retains **glass, oak (wood), and iron** in a growing set. The [adaptive checkpoint](docs/adaptive-checkpoint.md) records 13 passing CTest executables, bounded reference accuracy control, a grazing-contact guard and 12 full-size glass/oak/iron runs with matched node trajectories. Global convergence, calibrated interfaces and default-runtime integration remain open; observed costs are far from real time. The [material workshop](docs/creator-checkpoint.md) now collects inventory, validates manual Codex proposals, builds physical rigid spheres and saves recipes/state. Its CLI, viewer and glass/oak/iron tests share the compiler; all 14 Windows suites pass. Automatic in-app model connection, safe revisions and another geometry are the next application milestones. The earlier rigid restitution failure remains documented.
...
After building, run `banjo_workshop --workspace build/my-workshop` (Windows Release executable: `build/win-integration/Release/banjo_workshop.exe`). Collect a material, review a sphere design and its cost, Build, then Run. The assistant button currently exchanges request/proposal files with an external assistant; it does not automatically call a model. [The creator guide](docs/creator-checkpoint.md) includes the file contract, saved-world behavior, supported limits and three-material CLI fixtures.
