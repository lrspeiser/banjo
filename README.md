...
The [mechanics scorecard](docs/mechanics-scorecard.md) tracks every retained mechanic and platform capability with evidence and next steps. Material-dependent testing retains **glass, oak (wood), and iron** in a growing set. The [explicit-compliance checkpoint](docs/compliance-checkpoint.md) records 12 passing CTest executables and 27 full-resolution runs across those three substances, including gravity-loaded contact. Analytical contact/work checks pass; sampled trajectory convergence, calibrated interface laws and default-runtime integration remain open. The earlier rigid restitution failure remains documented.
