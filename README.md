...
The [mechanics scorecard](docs/mechanics-scorecard.md) tracks every retained mechanic and platform capability with evidence and next steps. Material-dependent testing retains **glass, oak (wood), and iron** in a growing set. The [contact-root checkpoint](docs/event-root-checkpoint.md) records 11 passing CTest executables, full-resolution elastic impacts using short intervals, and unresolved dissipative-contact, timestep-accuracy and performance limits.
