...
The [mechanics scorecard](docs/mechanics-scorecard.md) tracks every retained mechanic and platform capability with evidence and next steps. Material-dependent testing retains **glass, oak (wood), and iron** in a growing set. The [adaptive checkpoint](docs/adaptive-checkpoint.md) records 13 passing CTest executables, bounded reference accuracy control, a grazing-contact guard and 12 full-size glass/oak/iron runs with matched node trajectories. Global convergence, calibrated interfaces and default-runtime integration remain open; observed costs are far from real time. The next milestone is an [inventory-backed creator workbench](docs/creator-loop.md): ask an LLM to make a supported object from collected materials, then test and revise it. The same object/compiler path will retain the three-material experiments; this creator workflow is still planned. The earlier rigid restitution failure remains documented.
