"""Workshop component API with persisted visual-skin overrides.

The tested component implementation remains in ``workshop_components_core``.
This compatibility layer widens component overrides with one extra, orthogonal
field: ``skin``.  Core geometry ignores that field; the Workshop visual compiler
...
def checked_overrides(value: Any) -> dict[str, dict[str, Any]]:
    ...
    for name, patch in value.items():
        ...
        stripped[str(name)] = {k: deepcopy(v) for k, v in patch.items() if k != "skin"}
        if "skin" in patch:
...
def _changed(part, patch):
    return _BASE_CHANGED(part, {k: v for k, v in patch.items() if k != "skin"})


# The core functions resolve these globals at call time, so both direct callers
# and old imports automatically accept/preserve skin overrides.
