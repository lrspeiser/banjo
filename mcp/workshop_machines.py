...
def installed(design: Any, component_to_body: dict[str, str]) -> dict[str, Any]:
    ...
    sits on whichever body its component ended up in.
    """
    record = of(design)
    if not record:
        return {}
    ...
    if record.get("panels"):
        out["panels"] = [{"name": p["name"], "body": body(p["on"], "a panel"), "store": p["store"],
