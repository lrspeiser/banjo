...
int main(int argc,char **argv){
    try{
        unsigned chunks=4096,regions=4,frames=600,work=32768;std::string output,package;
        for(int i=1;i<argc;++i){
            const std::string_view option=argv[i];
            if(option=="--help"){
                std::cout<<"banjo_world_cli [--package file.json | --chunks 4096 --regions 4] [--frames 600] [--work 32768] [--output file.json]\nInsulated thermal/reaction/phase coupons. Cold storage is not full-world simulation.\n";return 0;
            }
