...
int main(int argc,char **argv){try {
    if(argc<2||argc>4)throw std::invalid_argument("usage: banjo_runtime_cohesive_probe OUTPUT.csv [--require-accuracy] [--jolt-surfaces]");
    bool require_accuracy=false,jolt_surfaces=false;
    for(int i=2;i<argc;++i){const std::string option=argv[i];if(option=="--require-accuracy"&&!require_accuracy)require_accuracy=true;else if(option=="--jolt-surfaces"&&!jolt_surfaces)jolt_surfaces=true;else throw std::invalid_argument("unknown or duplicate probe option");}
    unsigned failures=0;
    std::ofstream out(argv[1]);if(!out)throw std::runtime_error("cannot open report");
    out<<std::setprecision(17)<<"material,position_bits,offset_m,mode,steps,duration_s,opening_m,separated,damage_j,initial_energy_j,max_energy_error_j,transfer_error_sum_j,absolute_transfer_error_sum_j,max_splitting_error_j,max_momentum_error,oracle_relative_error,jolt_surfaces,contact_events\n";
    ...
    for(double offset:{0.,10.})for(bool fracture:{false,true})for(unsigned count:{128u,256u,512u,1024u}) {
        ...
        if(!jolt_surfaces)world.setPairContactOwner(1,2,PairContactOwner::External);
        ...
        const double dt=static_cast<float>(duration/count);double max_e=0,transfer_error=0,absolute_transfer_error=0,max_splitting_error=0,max_p=0;unsigned contact_events=0;
        const auto kick=[&](double force) {
            const auto sa=world.snapshot(1),sb=world.snapshot(2);
            PairImpulseAudit receipt;
            if(jolt_surfaces) {
                const auto step=world.applyCohesiveTensionKick(1,2,{side/2,0,0},{-side/2,0,0},reference-side,law,history,dt/2,std::max(1e-12,work*1e-4));
                history=step.interface_increment.state;response=step.interface_increment.response;receipt=step.transfer;
            } else receipt=world.applyPairImpulse(1,2,sa.center_of_mass_world_m+Vec3{side/2,0,0},
        ...
        for(unsigned i=0;i<count;++i) {
            ...
            const auto impacts=world.drainImpacts();contact_events+=static_cast<unsigned>(impacts.size());
            if(!jolt_surfaces&&!impacts.empty())throw std::runtime_error("externally owned connector received a Jolt impact");
        }
        ...
        out<<materialPresetName(preset)<<','<<8*sizeof(JPH::Real)<<','<<offset<<','<<(fracture?"separation":"elastic")<<','<<count<<','<<dt*count<<','<<history.opening_m<<','<<response.separated<<','<<response.dissipated_energy_j<<','<<e0<<','<<max_e<<','<<transfer_error<<','<<absolute_transfer_error<<','<<max_splitting_error<<','<<max_p<<','<<oracle_error<<','<<jolt_surfaces<<','<<contact_events<<'\n';
        if(count==1024) {
    ...
    return require_accuracy&&failures?2:0;
