...
int main(int argc,char **argv){try{
    ...
    PlatformPlayback playback;std::size_t selected=0;bool running=false,reference=false;double accumulator=0,time=0,lastStep=0;std::string message;unsigned broken=0;
    ...
    auto load=[&]{playback.clear();running=false;accumulator=0;time=0;broken=0;std::ifstream f(files[selected]);if(std::filesystem::file_size(files[selected])>4194304)throw std::runtime_error("package too large");std::string source((std::istreambuf_iterator<char>(f)),{});auto candidate=PlatformWorld::load(source);auto definition=nlohmann::json::parse(candidate->packageJson());reference=definition["backend"]=="bonded-reference-v2";world=std::move(candidate);instances=world->renderInstances();triangles=world->supportMesh();if(reference)playback.start(world->packageJson());message=reference?"Calculating full physics; replay will run at normal speed":"Live rigid runtime: fracture unsupported";};load();
    ...
    while(!WindowShouldClose()){
        ...
        playback.poll(GetFrameTime());
        if(reference){
            if(auto frame=playback.frame()){time=frame->time_s;instances=frame->instances;broken=frame->broken_bonds;}
            if(playback.computing())message=TextFormat("Calculating physics: %.0f%%. Window remains responsive.",playback.progress()*100);
            else if(!playback.error().empty())message=playback.error();
            else if(playback.ready())message="Computed trajectory / normal-speed replay. Click Replay.";
        }else if(running){
            accumulator+=std::min(double(GetFrameTime()),.1);unsigned steps=0;
            while(accumulator>=world->fixedStep()&&steps++<24){auto r=world->step();time=r.elapsed_s;lastStep=r.wall_ms;accumulator-=world->fixedStep();if(!r.error.empty()){message=r.error;running=false;break;}}
        ...
        if(button(72,reference?(playback.computing()?"Cancel calculation":playback.playing()?"Pause":playback.ready()?"Replay":"Calculate"):(running?"Pause":"Run"))||IsKeyPressed(KEY_SPACE)){
            if(reference){if(playback.computing()){playback.clear();message="Calculation cancelled. Click Calculate to retry.";}else if(playback.playing())playback.pause();else if(playback.ready())playback.play();else playback.start(world->packageJson());}
            else {running=!running;accumulator=0;}
        }
        ...
        if(button(232,reference?"Export final computed report":"Export report (platform-report.json)")&&(!reference||playback.ready())){
            std::ofstream f("build/platform-report.json");auto report=nlohmann::json::parse(reference?playback.finalReport():world->reportJson());
            if(reference){report["presentation"]="final computed state; replay may show an earlier frame";report["displayed_time_s"]=time;}
            f<<report.dump(2);message=f?"Report saved to build/platform-report.json":"Report write failed";
        }
        ...
        DrawFPS(980,790);
        DrawText(reference?"COMPUTED / REPLAY":"RIGID / LIVE",980,291,20,{203,232,126,255});
        ...
        DrawText(reference?"Calculate once; replay at normal speed":"Offset comes from starting geometry",980,559,15,{164,186,197,255});
    ...
    playback.clear();UnloadRenderTexture(viewport);CloseWindow();return 0;
