...
int banjo_make_blade(banjo_world *world, const char *body, const double heel_m[3],
    ...
    return guarded([&] {
        ...
        if (id == 0) {
            setError(std::string("\"") + body + "\" cannot take that edge: it is not in the "
                     "scene, the edge does not lie on its matter, the facing runs along the "
                     "edge, or a thickness, radius or bevel is out of range");
            return static_cast<int>(BANJO_BAD_ARGUMENT);
        }
