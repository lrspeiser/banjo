...
struct AttachmentImpulse { Vec3 point_a_m{},point_b_m{},impulse_on_a_n_s{}; };
// All sites act on one shared before/after state, including cross-site work.
inline AttachmentImpulseResult applyAttachmentImpulses(const AttachmentBody &a,const AttachmentBody &b,
                                                       const std::vector<AttachmentImpulse> &sites) {
    if(sites.empty()||sites.size()>256)throw std::invalid_argument("attachment site count must be 1..256");
    ...
    Vec3 total{},torque_a{},torque_b{};
    for(const auto &site:sites) {
        const auto pa=site.point_a_m,pb=site.point_b_m,j=site.impulse_on_a_n_s;
        if(!finite(pa)||!finite(pb)||!finite(j))throw std::invalid_argument("invalid attachment impulse/point");
        total+=j;torque_a+=cross(pa-a.center_m,j);torque_b+=cross(pb-b.center_m,j);
        out.angular_impulse_kg_m2_s+=cross(pa-pb,j);
    }
    out.a.velocity_m_s+=total/a.mass_kg;out.b.velocity_m_s-=total/b.mass_kg;
    out.a.angular_velocity_rad_s+=ia*torque_a;out.b.angular_velocity_rad_s-=ib*torque_b;
    ...
    for(const auto &site:sites) {
        const auto ra=site.point_a_m-a.center_m,rb=site.point_b_m-b.center_m;
        out.impulse_work_j+=.5*dot(site.impulse_on_a_n_s,velocity(a,ra)+velocity(out.a,ra)-velocity(b,rb)-velocity(out.b,rb));
    }
...
inline AttachmentImpulseResult applyAttachmentImpulse(const AttachmentBody &a,const AttachmentBody &b,
    Vec3 pa,Vec3 pb,Vec3 j) {
    return applyAttachmentImpulses(a,b,{{pa,pb,j}});
}
}
