...
CohesiveRigidPairResult advanceCohesiveRigidPair(const CohesiveInterfaceLaw &law,double rest,const CohesiveRigidPairState &initial,double dt){
    ...
    if(!std::isfinite(measure)||(!response.separated&&measure>.1)||dt*std::max(length(omega(initial.a)),length(omega(initial.b)))>.1)throw std::invalid_argument("rigid cohesive timestep needs refinement");
