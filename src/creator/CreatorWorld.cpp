...
Json recipe(const ObjectRecipe &r) {
    return {{"schema_version",r.schema_version},{"name",r.name},{"shape",{{"type",r.shape},{"radius_m",r.radius_m}}},
...
ObjectRecipe recipe(const Json &j) {
    ...
    fields(j.at("shape"),{"type","radius_m"});fields(j.at("placement"),{"tangent_m","bitangent_m","clearance_m"});
    fields(j.at("motion"),{"linear_velocity_m_s","angular_velocity_rad_s"});
    return {static_cast<unsigned>(integer(j.at("schema_version"),100)),string(j.at("name"),80),string(j.at("shape").at("type")),
        string(j.at("physics")),material(j.at("material")),number(j.at("shape").at("radius_m")),
...
CreationPreview compile(const ObjectRecipe &r,const CreatorSettings &settings) {
    check(r.schema_version==1,"unsupported object schema version");check(r.shape=="sphere","unsupported shape; this version supports sphere");
    ...
    check(std::isfinite(r.radius_m)&&r.radius_m>=.025&&r.radius_m<=.5,"sphere radius must be 0.025–0.5 m");
    check(std::isfinite(r.tangent_m)&&std::abs(r.tangent_m)<=8-r.radius_m&&std::isfinite(r.bitangent_m)&&std::abs(r.bitangent_m)<=3-r.radius_m,
    ...
    const double volume=4.0/3*std::numbers::pi*r.radius_m*r.radius_m*r.radius_m;
    const double mass=volume*makeReferenceMaterial(r.material).density_kg_m3;
    return {r,volume,mass,.4*mass*r.radius_m*r.radius_m,
        pointInPlaneFrame(makeSupportPlaneFromSlopeDegrees(settings.slope_degrees),r.tangent_m,r.bitangent_m,r.radius_m+r.clearance_m),{}};
}
Json signature() {
    ...
    return {{"object_compiler",1},{"runtime","jolt-5.6/banjo-rigid-v1"},{"profiles",profiles}};
}
...
Json previewJson(const CreationPreview &p) {
    return {{"recipe",recipe(p.recipe)},{"volume_m3",p.volume_m3},{"required_mass_kg",p.mass_kg},
        {"inertia_kg_m2",p.inertia_kg_m2},{"position_m",vector(p.position_world_m)},{"allocations",allocations(p.allocations)},
        {"approximation","intact rigid sphere; catalog contact coefficients are not calibrated substance behavior"}};
}
...
CreationPreview CreatorWorld::preview(const ObjectRecipe &r) const {
    ...
    for (const auto &object:objects_) check(length(object.state.center_of_mass_world_m-result.position_world_m)>object.recipe.radius_m+r.radius_m+.001,
...
MatterBodyId CreatorWorld::create(std::string request_id,const ObjectRecipe &r) {
    ...
    next_objects.push_back({id,std::move(request_id),r,plan.volume_m3,plan.mass_kg,plan.inertia_kg_m2,plan.allocations,
        {plan.position_world_m,{},r.linear_velocity_m_s,r.angular_velocity_rad_s}});
    ...
    world_->addBall({.body_id=id,.radius_m=r.radius_m,.material=makeReferenceMaterial(r.material),.position_world_m=plan.position_world_m,
        .linear_velocity_m_s=r.linear_velocity_m_s,.angular_velocity_rad_s=r.angular_velocity_rad_s,.mass_override_kg=plan.mass_kg});
...
std::string CreatorWorld::serialize() const {
    ...
    return Json{{"world_version",1},{"physics_signature",signature()},
...
CreatorWorld CreatorWorld::deserialize(std::string_view document) {
    ...
    check(integer(j.at("world_version"),100)==1,"incompatible world version");check(j.at("physics_signature")==signature(),"saved material/runtime signature differs; explicit migration is required");
    ...
    for (const auto &value:objects) {
        ...
        CreatedObject object{integer(value.at("id"),max_objects),string(value.at("request_id")),r,p.volume_m3,p.mass_kg,p.inertia_kg_m2,{},state(value.at("state"))};
    ...
    for (const auto &object:world.objects_) {
        world.world_->addBall({.body_id=object.id,.radius_m=object.recipe.radius_m,.material=makeReferenceMaterial(object.recipe.material),
            .position_world_m=object.state.center_of_mass_world_m,.linear_velocity_m_s=object.state.linear_velocity_m_s,
            .angular_velocity_rad_s=object.state.angular_velocity_rad_s,.mass_override_kg=object.mass_kg});
        world.world_->applySphereContactState(object.id,{object.state,object.recipe.radius_m,object.mass_kg,object.inertia_kg_m2});
    }
    return world;
}
...
std::string CreatorWorld::inspectJson() const {
    ...
    result["capabilities"]={{"shapes",Json::array({"sphere"})},{"physics",Json::array({"rigid-v1"})},
        {"unsupported",Json::array({"deformation","fracture","wood grain","metal plasticity","assemblies"})},
        {"limits",{{"objects",max_objects},{"radius_m",Json::array({.025,.5})},{"initial_speed_m_s",5},{"initial_spin_rad_s",50}}}};
    ...
    for (std::size_t i=0;i<objects_.size();++i) {
        ...
        const auto motion=measureRollingKinematics(object.state,object.recipe.radius_m,support(),insideSupportFootprint(support(),object.state.center_of_mass_world_m,8,3));
        const auto totals=measureRigidMechanics(world_->mechanicalState(object.id),settings_.gravity_m_s2);
        j["mass_kg"]=object.mass_kg;j["volume_m3"]=object.volume_m3;j["inertia_kg_m2"]=object.inertia_kg_m2;
        j["motion_state"]=rollingStateName(motion.state);j["contact_slip_m_s"]=motion.contact_slip_speed_m_s;
