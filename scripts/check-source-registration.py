...
INTENTIONALLY_UNBUILT = {
    'src/physics/TetrahedronContact.cpp':
        'The tetrahedron-tetrahedron contact query does not pass its own test: GJK '
        'returns no witness points for separated tetrahedra and Indeterminate for '
        'exactly-touching faces, so 2 of 4 cases fail. It is Jolt-dependent and '
        'belongs in banjo_runtime, not the Jolt-free banjo_core, when it is fixed. '
        'Measured defects and the next step are in '
        'docs/tetrahedron-contact-checkpoint.md.',
    'tests/tetrahedron_contact_tests.cpp':
        'The test for the unbuilt src/physics/TetrahedronContact.cpp. Registering it '
        'now would add a failing ctest entry; its tolerances must not be relaxed to '
        'make it pass. See docs/tetrahedron-contact-checkpoint.md.',
}
