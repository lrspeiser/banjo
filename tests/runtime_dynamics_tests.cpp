...
void frictionlessSurfaceCannotSpinUpBall() {
    ...
    const double surface_spin_drift = .25 * banjo::length(body.angular_velocity_rad_s);
    const double initial_speed = 2.0;
    // Jolt's single-precision normal/torque-arm arithmetic accumulates small
    // platform-dependent spin. At this checkpoint Linux measured 6.53 um/s,
    // MSVC 11.22 um/s after 120 steps. Bound drift to 8 ppm of launch speed
    // (16 um/s here); the former 10 um/s bound excluded the Windows baseline.
    // This is a numerical regression budget, not a claim of exact zero torque.
    const double drift_tolerance = 8.0e-6 * initial_speed;
    std::cout << "Frictionless angular drift=" << banjo::length(body.angular_velocity_rad_s)
              << " rad/s; surface drift=" << surface_spin_drift
              << " m/s; bound=" << drift_tolerance << " m/s\n";
    require(surface_spin_drift < drift_tolerance,
        "surface without friction must not manufacture rolling spin");
    const double rotational_energy_fraction =
        .4 * surface_spin_drift * surface_spin_drift / (initial_speed * initial_speed);
    require(rotational_energy_fraction < 3.0e-11,
        "frictionless numerical spin energy must remain negligible relative to launch energy");
}
