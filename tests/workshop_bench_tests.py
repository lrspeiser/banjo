"""Fast contracts for Workshop's isolated functional test bench."""
...
class App:
    def __init__(self, root: Path) -> None:
        ...
        self.live = object()  # must never be borrowed by a Workshop trial
    ...
    def send(self, **command):
        ...
        if op == "thermo":
            ...
            return {"ok": True, "thermo": {"bodies": [
                {"name": "water charge", "temperature_k": t,
                 "contents_kg": {"moisture": 1.0}},
                {"name": "kettle bottom", "temperature_k": 350.0},
                {"name": "heater plate", "temperature_k": 500.0},
            ], "ledger": {"heater_in_j": 216000.0, "residual_j": 0.0}}}
        if op == "step":
            self.state = {"ok": True, "t": self.state["t"] + command.get("dt", 0) * command.get("n", 1),
                          "bodies": []}
            return self.state
...
class FakeMachineSession:
    def __init__(self):
        ...
        self.state = {
            "t": 0.0,
            "bodies": [{"name": "load", "position_m": [0.1, 0.45, 0.0]}],
    ...
    def send(self, **command):
        op = command.get("op")
        if op == "operate":
            self.state["machines"]["controls"] = [{
                "id": 7, "name": "hoist", "power": command.get("power"),
...
class FakeLive:
    ...
    def __init__(self):
        self.session = FakeMachineSession()
        self.closed = False
        FakeLive.last = self
...
class BenchCatalog(unittest.TestCase):
    def test_kettle_and_machine_are_editable_bench_fixtures(self):
        tests = {t["test"]: t for t in workshop_bench.catalog()}
        self.assertEqual({"kettle_heat", "machine_control"}, set(tests))
...
class IsolatedBench(unittest.TestCase):
    def setUp(self):
        ...
        self.design = assemble("table", design_id="candidate")
    ...
    def test_kettle_reports_engine_temperature_and_ledger_without_the_outside_world(self):
        ...
        with mock.patch.object(workshop_bench.live_session, "Session", FakeKettleSession):
            result = workshop_bench.run(self.app, self.design, {
                "test": "kettle_heat", "config": {"water_kg": 1.0, "duration_s": 120,
                                                    "heater_power_w": 1800}})
        ...
        with mock.patch.object(workshop_bench.live_session, "Live", FakeLive):
            result = workshop_bench.run(self.app, self.design, {
                "test": "machine_control", "config": {"power": True, "direction": -1,
                                                        "setting": 35, "duration_s": 0.5,
                                                        "load_kg": 20}})
        self.assertIs(self.app.live, outside)
        self.assertTrue(FakeLive.last.closed)
        self.assertEqual("applied", result["measured"]["acknowledgement"])
        control = result["measured"]["control"]
        self.assertEqual(-1, control["direction"])
        self.assertAlmostEqual(0.35, control["setting"])
        self.assertNotEqual(0.0, result["measured"]["load_delta_y_m"])

    def test_bench_presets_are_owner_scoped_and_persistent(self):
        one = workshop_library.save_bench_preset(
            self.app, name="Half speed lift", test_name="machine_control",
            config={"power": True, "direction": 1, "setting": 50})
        self.assertEqual("machine_control", one["test"])
        again = workshop_library.list_bench_presets(self.app)
        self.assertEqual("Half speed lift", again[0]["name"])
        other = App(self.root)
        other.workshop_owner_id = "someone-else"
