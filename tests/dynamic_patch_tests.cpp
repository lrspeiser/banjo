...
void requireReportMatchesFreshEvaluation(const DynamicPatch &dynamic, std::string_view message) {
    const auto cached = dynamic.report();
    const auto fresh = dynamic.patch().evaluate(dynamic.patch().state().displacements_m);
    near(cached.stored_free_energy_j, fresh.stored_free_energy_j, 1.e-15, message);
    near(cached.plastic_dissipation_j, fresh.plastic_dissipation_j, 1.e-15, message);
    near(cached.maximum_displacement_gradient_norm, fresh.maximum_displacement_gradient_norm,
         1.e-15, message);
}
...
SmallStrainLaw j2Law() {
    ...
    const J2Material material{.young_modulus_pa = young,
    ...
    return {.kind = SmallStrainLawKind::J2Plastic,
            ...
            .young_modulus_pa = {young, young, young},
            .poisson_xy_yz_zx = {poisson, poisson, poisson},
            .shear_xy_yz_zx_pa = {young / (2 * (1 + poisson)), young / (2 * (1 + poisson)),
                                  young / (2 * (1 + poisson))},
            .maximum_total_strain_norm = .1};
}

PatchDefinition oneTet(SmallStrainLaw law, bool oscillator = false, double density = 1000) {
...
void uniformTranslationHasZeroStrainAndConstantMomentum() {
    ...
    nearVec(dynamic.report().linear_momentum_kg_m_s, dynamic.patch().massKg() * velocity, 1.e-11,
            "uniform translation momentum must remain constant");
    requireReportMatchesFreshEvaluation(
        dynamic, "elastic accepted evaluation cache must match fresh evaluation");
}
...
void freeFallMatchesSymplecticEulerMomentum() {
    ...
    nearVec(result.linear_momentum_kg_m_s,
            dynamic.patch().massKg() * load.gravity_m_s2 * (steps * dt), 1.e-10,
            "gravity impulse must equal momentum change");
...
void freeFallVelocityVerletIsExactForConstantGravity() {
    DynamicPatchOptions options;
    options.integrator = DynamicPatchIntegrator::VelocityVerlet;
    DynamicPatch dynamic(oneTet(elasticLaw()), options);
    DynamicPatchLoad load = zeroLoad(4);
    load.gravity_m_s2 = {0, -9.81, 0};
    const double dt = dynamic.stableTimeStepLimitS() * .2;
    constexpr unsigned steps = 12;
    ...
    for (unsigned i = 0; i < steps; ++i) {
        result = dynamic.step(dt, load);
        require(result.accepted, "velocity-Verlet free-fall step must be accepted");
    }
    const double time = steps * dt;
    for (std::size_t i = 0; i < 4; ++i) {
        near(dynamic.state().velocities_m_s[i].y, -9.81 * time, 1.e-13,
             "velocity-Verlet constant-gravity velocity must be exact");
        near(dynamic.patch().state().displacements_m[i].y, -.5 * 9.81 * time * time, 1.e-13,
             "velocity-Verlet constant-gravity position must be exact");
    }
    nearVec(result.linear_momentum_balance_residual_kg_m_s, {}, 1.e-12,
            "velocity-Verlet gravity momentum ledger must close to roundoff");
    requireReportMatchesFreshEvaluation(
        dynamic, "velocity-Verlet elastic cache must match fresh evaluation");
}
...
void rejectedStepsRollBackEveryStateField() {
    ...
    requireReportMatchesFreshEvaluation(dynamic, "rejection must retain accepted evaluation cache");
    ...
    try {
        dynamic.setVelocitiesMPerS(std::vector<Vec3>(4));
    } catch (const std::invalid_argument &) {
        rejected_late_velocity = true;
    }
    require(rejected_late_velocity,
            "velocity replacement after stepping must reject unaccounted momentum injection");
}

double oscillatorError(unsigned divisor, double duration, double reference_x, double reference_v) {
    DynamicPatch dynamic(oneTet(elasticLaw(), true));
    std::vector<Vec3> velocity(4);
    velocity[1].x = .01;
    ...
    return std::hypot(x - reference_x, dynamic.stableTimeStepLimitS() * (v - reference_v));
}
...
void elasticOscillationRefinesWithTimeStep() {
    ...
    std::cout << "[INFO] elastic oscillator dt limits: coarse=" << scale.stableTimeStepLimitS() / 4
              << " fine=" << scale.stableTimeStepLimitS() / 16 << " errors=" << coarse << ','
              << fine << '\n';
    require(fine < coarse,
            "elastic oscillator trajectory error must decrease under time refinement");
}

struct OscillatorResult {
    double trajectory_error{};
    double energy_error{};
};

OscillatorResult oscillatorResult(DynamicPatchIntegrator integrator, unsigned divisor,
                                  double duration, double omega) {
    DynamicPatchOptions options;
    options.integrator = integrator;
    DynamicPatch dynamic(oneTet(elasticLaw(), true), options);
    std::vector<Vec3> velocity(4);
    velocity[1].x = .01;
    dynamic.setVelocitiesMPerS(velocity);
    const double requested = dynamic.stableTimeStepLimitS() / divisor;
    const unsigned steps = static_cast<unsigned>(std::ceil(duration / requested));
    const double dt = duration / steps;
    const double initial_energy = .5 * (1000. / 24.) * .01 * .01;
    OscillatorResult result;
    for (unsigned i = 0; i < steps; ++i) {
        require(dynamic.step(dt, zeroLoad(4)).accepted,
                "integrator comparison oscillator step must be accepted");
        const double time = (i + 1) * dt;
        const double exact_x = .01 * std::sin(omega * time) / omega;
        const double exact_v = .01 * std::cos(omega * time);
        const double x = dynamic.patch().state().displacements_m[1].x;
        const double v = dynamic.state().velocities_m_s[1].x;
        const auto report = dynamic.report();
        result.trajectory_error =
            std::max(result.trajectory_error,
                     std::hypot(x - exact_x, dynamic.stableTimeStepLimitS() * (v - exact_v)));
        result.energy_error =
            std::max(result.energy_error, std::abs(report.kinetic_energy_j +
                                                   report.stored_free_energy_j - initial_energy));
    }
    return result;
}

void velocityVerletImprovesHarmonicOscillatorAccuracy() {
    DynamicPatch scale(oneTet(elasticLaw(), true));
    constexpr double young = 1.e6, poisson = .25, density = 1000;
    const double lambda = young * poisson / ((1 + poisson) * (1 - 2 * poisson));
    const double mu = young / (2 * (1 + poisson));
    const double omega = std::sqrt(4 * (lambda + 2 * mu) / density);
    const double duration = scale.stableTimeStepLimitS() * 20;
    const auto symplectic =
        oscillatorResult(DynamicPatchIntegrator::SymplecticEuler, 8, duration, omega);
    const auto verlet =
        oscillatorResult(DynamicPatchIntegrator::VelocityVerlet, 8, duration, omega);
    const auto verlet_fine =
        oscillatorResult(DynamicPatchIntegrator::VelocityVerlet, 16, duration, omega);
    std::cout << "[INFO] matched oscillator errors symplectic=(" << symplectic.trajectory_error
              << ',' << symplectic.energy_error << ") verlet=(" << verlet.trajectory_error << ','
              << verlet.energy_error << ") verlet-fine=(" << verlet_fine.trajectory_error << ','
              << verlet_fine.energy_error << ")\n";
    require(verlet.trajectory_error < symplectic.trajectory_error,
            "velocity-Verlet must improve harmonic trajectory accuracy at matched dt");
    require(verlet.energy_error < symplectic.energy_error,
            "velocity-Verlet must improve harmonic energy error at matched dt");
    require(verlet_fine.trajectory_error < verlet.trajectory_error &&
                verlet_fine.energy_error < verlet.energy_error,
            "velocity-Verlet harmonic trajectory and energy errors must refine with dt");
}
...
void matchedMaterialTranslationControls() {
    ...
    for (const auto &entry : cases) {
        ...
        nearVec(result.linear_momentum_kg_m_s, dynamic.patch().massKg() * velocity, 1.e-12,
                "matched material translation momentum");
        std::cout << "[INFO] " << entry.name
                  << " stable dt limit=" << dynamic.stableTimeStepLimitS() << " s\n";
    }
}
...
void j2StepCommitsPositivePlasticHistory() {
    ...
    requireReportMatchesFreshEvaluation(
        dynamic, "J2 accepted evaluation cache must match constitutive state");
}
...
int main() {
    const std::vector<std::pair<std::string_view, std::function<void()>>> tests{
        ...
        {"velocity-Verlet constant gravity", freeFallVelocityVerletIsExactForConstantGravity},
        ...
        {"velocity-Verlet harmonic accuracy", velocityVerletImprovesHarmonicOscillatorAccuracy},
