...
int main(){try{
    ...
    for(auto material:{MaterialPreset::Glass,MaterialPreset::Oak,MaterialPreset::Iron}){
        BowlLab lab;BowlSettings rigid;rigid.experimental_fracture=false;lab.configure(rigid);rejected=false;try{(void)lab.craft(material);}catch(const std::invalid_argument&){rejected=true;}check(rejected&&lab.stock().objects().empty(),"uncollected stock cannot craft");
        ...
        lab.configure(rigid);check(lab.stock().inventoryMass(material)==inventory&&lab.timeSeconds()==0,"reset preserves allocations");
    ...
    BowlLab mixed;BowlSettings rigid;rigid.experimental_fracture=false;mixed.configure(rigid);for(int round=0;round<2;++round)for(auto m:{MaterialPreset::Glass,MaterialPreset::Oak,MaterialPreset::Iron}){mixed.collect(m);(void)mixed.craft(m);}
    ...
    for(auto surface:{MaterialPreset::Concrete,MaterialPreset::Glass,MaterialPreset::Oak,MaterialPreset::Iron})for(double tilt:{0.0,10.0}){
        BowlSettings setup;setup.experimental_fracture=false;setup.surface=surface;setup.tilt_degrees=tilt;mixed.configure(setup);
