    ...
    def test_a_hoist_winds_its_rope_on_lifts_the_crate_and_holds_it_braked(self):
        ...
        with banjo.World(hoist_room(), cell_size_m=0.05) as world:
            ...
            self.assertEqual((hanging.id, hanging.drum, hanging.load, hanging.radius_m,
                              hanging.length_m, hanging.attached, hanging.winds),
                             (rope, "drum", "crate", radius, 2.0, True, 1))
...
class AWorldIsKeptFromPython(unittest.TestCase):
    ...
    def test_a_saved_hoist_opens_again_with_its_battery_its_motor_and_its_brake(self):
        centre, axle = (0.0, 2.0, 0.0), (0.0, 0.0, 1.0)
        with banjo.World(hoist_room(), cell_size_m=0.05) as world:
            pin = world.hinge("post", "drum", at_m=centre, axis=axle)
            world.drum("drum", "crate", centre_m=centre, axis=axle, radius_m=0.1,
                       load_at_m=(0.1, 0.5, 0.0), winds=1, length_m=2.0)
            battery = world.energy_store("battery", "post", 5000.0, 5000.0)
            drive = world.motor(pin, battery, 60.0, 10.0, brake_torque_n_m=200.0)
            # Wind some rope on, and hold the crate up on the brake.
            world.drive_motor(drive, 1.0)
            run(world, 0.5)
            world.drive_motor(drive, 0.0, brake=True)
            run(world, 0.5)
            saved = world.snapshot("the hoist")
            self.assertIsNotNone(saved, world.last_refusal)
            stores = world.energy_stores()
            (motor,) = world.motors()
            (rope,) = world.drum_ropes()
            up = world.body("crate").position_m[1]
        with banjo.World(hoist_room(), cell_size_m=0.05, snapshot=saved) as again:
            said = again.restored()
            self.assertEqual(said["tier"], "whole", said.get("why"))
            # The battery as it was; the motor whole -- its command, brake and
            # account, and what it said of its last step, until it takes another;
            # and how much rope is out.
            self.assertEqual(again.energy_stores(), stores)
            (kept,) = again.motors()
            (hung,) = again.drum_ropes()
            self.assertEqual(kept, motor)
            self.assertEqual(kept.state, "braking")
            on_the_drum = ("id", "drum", "load", "radius_m", "length_m", "out_m", "wound_m",
                           "attached", "winds")
            self.assertEqual({f: getattr(hung, f) for f in on_the_drum},
                             {f: getattr(rope, f) for f in on_the_drum})
            print(f"\n  opened again: the battery has {stores[0].charge_j:.3f} J, the motor is "
                  f"{kept.state} having drawn {kept.drawn_j:.3f} J, and {hung.out_m:.6f} m of rope "
                  f"is out")
            # Still on its brake, it holds the crate where it was ...
            run(again, 1.0)
            held = again.body("crate").position_m[1]
            print(f"  in a second on its brake the crate moved {held - up:.6f} m")
            self.assertLess(abs(held - up), 0.002, "opened again, the brake did not hold the crate")
            # ... and told to, it goes on winding, drawing on the same battery.
            again.drive_motor(drive, 1.0)
            run(again, 0.5)
            (going,) = again.motors()
            store = again.energy_stores()[0]
            risen = again.body("crate").position_m[1] - held
            print(f"  wound on for 0.5 s, the crate rose {risen:.4f} m; the motor has drawn "
                  f"{going.drawn_j:.3f} J in all and the battery has given {store.given_j:.3f} J")
            self.assertGreater(risen, 0.05, "opened again, the hoist did not wind")
            self.assertLessEqual(abs(store.given_j - going.drawn_j), 1e-9 * going.drawn_j,
                                 "opened again, the battery did not give what the motor drew")
