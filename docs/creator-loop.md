...
Current selected-revision extension: [source `14bb0b7` and evidence](revision-checkpoint.md) supports preview/rebuild/reclaim, identity/revision/history and real glass/oak/iron material reuse. Five real provider cases and all 15 suites pass; workshop build/capture pass, normal control verification remains pending. Next: consistent API receipts/retries/durability, inventory energy reference states and bounded functional tests.

Energy application requirement: a creation must ultimately use **materials + an energy source + a capable tool/process**. The [energy-system plan](energy-system.md) adds a visible work/energy bill, power limits, combined transaction receipts and physically consistent breakage/recovery. The current intact-authoring sandbox has no energy cost or manufacturing law; do not treat its full material recovery as an energy refund or as the final construction process.

Previous shape extension: [oriented solid boxes](shape-checkpoint.md) now share the sphere inventory/compiler/LLM path. All three reference substances pass geometry/mass/inertia/dynamics checks; real model calls produce buildable blocks. Capture passes, while normal new-control verification remains pending. Safe revisions/reclaimed material remain next.
