...
Make this creator loop the application-facing purpose of the next shared experiment workbench. First establish inventory/specification/compiler boundaries and one supported object creation path; connect the audited simulation with synchronized ownership; then add work-accounted friction and the rolling demonstration. Keep the user-visible create/test/revise path in each subsequent material and shape milestone. Do not wait until every material family or Gate 8 is complete to exercise that path, and do not interpret it as permission to hide the open conservation, performance or fracture defects.

Current status: **planned; not yet implemented end to end**. See the [mechanics/platform scorecard](mechanics-scorecard.md), [current reference evidence](adaptive-checkpoint.md), [project master plan](project-master-plan.md) and [roadmap](roadmap.md).
