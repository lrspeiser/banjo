...
Application priority: the [first creator loop](creator-loop.md) is a **working rigid-sphere prototype with an automatic Windows Codex adapter**. Collection, validated proposal/preview, atomic creation, physical testing and saved state work through the same compiler as glass/oak/iron tests. Next support another shape and account for revisions/reclaimed material. Carry this application through foundation work.
...
| P06 Human/AI creator APIs | Prototype: automatic Windows Codex proposal/clarification → independently validated preview → physical object; glass/oak/iron real calls and no-build door case | Second geometry, broader semantic/multi-turn evaluation and resource-aware modification of existing objects; other provider/platform adapters remain open |
...
| P10 Usable shared laboratory and visualization | Measured automatic Windows collection/request/cancel/retry/build/run/pause/save, typed input, camera and capture; 15 suites pass | Comparative shape work and safe revision controls; detailed reference integration still needs accepted-state/budget feedback |

## Current order of work

1. Extend the working automatic creator path with a second shape and safe revisions with resource accounting. Preserve independently validated previews, shared glass/oak/iron fixtures, one clock/contact owner and visible capability/budget limits.
...
Evidence: [automatic assistant checkpoint](assistant-checkpoint.md), [creator workshop checkpoint](creator-checkpoint.md), [adaptive/12-run checkpoint](adaptive-checkpoint.md), [full-state/five-rate checkpoint](trajectory-checkpoint.md), [explicit compliance/27-case checkpoint](compliance-checkpoint.md), [contact-root/full-resolution checkpoint](event-root-checkpoint.md), [event/material comparison](event-material-checkpoint.md), [support/timestep checkpoint](coupled-support-checkpoint.md), [elastic reference](elastic-newton-checkpoint.md), [default-stage defects](material-stage-checkpoint.md), [transfer accounting](transfer-accounting-checkpoint.md), [property consumers](physics-coverage.md), [ordered roadmap](roadmap.md). The table records the scope of that evidence, including missing evidence, rather than inferring completion from passing suites.
