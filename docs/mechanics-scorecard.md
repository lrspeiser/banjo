...
Application priority: the [first creator loop](creator-loop.md) is a **sphere/box prototype with an automatic Windows Codex adapter**. Collection, validated proposal/preview, atomic creation, physical testing and saved state work through the same compiler as glass/oak/iron tests. Next verify the new box controls normally and account for revisions/reclaimed material. Carry this application through foundation work.
...
| P05 Units, extensible laws and capability checks | Prototype strict SI sphere/box schema, full dimensions/world quaternion, material-derived tensors and capability/budget checks | Material fields, additional geometry and declared laws; preserve explicit unsupported behavior |
| P06 Human/AI creator APIs | Prototype automatic sphere/solid-box proposals through one validated compiler; real glass/oak/iron block calls and no-build hinged-door clarification | Normal box UI check; safe resource-aware object revisions and broader semantic/multi-turn evaluation |
| P07 Persistence and material history | Prototype format-2 sphere/box recipes/orientation/allocations/state and replay; exact known-v1 sphere migration retains matter/state | Safe revision/reclamation, damage/assembly lineage and broader migrations; no bit-exact restart or multiplayer claim |
...
| P10 Usable shared laboratory and visualization | Previous sphere UI verified; new six-object shape capture and 15 suites pass, but normal box-control verification pending native helper failure | Verify box request/edit/build/run/save when native access works; then safe revisions and detailed-reference feedback |

## Current order of work

1. Finish normal box-control verification, then extend the automatic creator path with safe revisions and material reclamation. Preserve independently validated previews, shared glass/oak/iron fixtures, one clock/contact owner and visible capability/budget limits.
