...
Living scorecard for the general-object platform. Latest tested code is `9cd9197bb99492b1a834622cd87d35302468c4dd` on local `codex/physics-foundation`; GitHub main remains separate. The [creator checkpoint](creator-checkpoint.md) records 14 passing Windows CTest executables, shared inventory-backed creation/CLI/workshop, a real manual Codex proposal and saved rigid state. The earlier [adaptive checkpoint](adaptive-checkpoint.md) records bounded local error control, a grazing-contact guard and 12 full-size glass/oak/iron runs with 15 matched comparisons. Local indicators do not certify global convergence, and the reference remains far from real time. The default viewer and fracture defects remain current. Update this document at every physics checkpoint, including failed and unsupported cases.
...
Application priority: the [first creator loop](creator-loop.md) is a **working rigid-sphere prototype with a manual Codex file bridge**. Collection, validated proposal/preview, atomic creation, physical testing and saved state work through the same compiler as glass/oak/iron tests. Next connect the assistant automatically, support another shape and account for revisions/reclaimed material. Carry this application through foundation work.
...
|---|---|---|
| M01 Matter-derived mass, volume, COM | Measured: sampled/transfer tests plus exact solid-sphere compiler and glass/oak/iron inventory/mass ledger | Extend the same creator/compiler path to hollow, non-spherical and heterogeneous objects |
| M02 Full inertia and finite-cell spin | Measured: spinning transfers include subcell inertia/spin; creator derives solid-sphere 2mr²/5 | Extend to full tensors for authored shapes, anisotropic cells and debris dynamics |
...
| M24 Shape and mass-distribution effects | Prototype: solid-sphere creator and generated convex fragments | Add box/cylinder through shared creation path, then hollow spheres, ellipsoids and composites with actual inertia/contact |
...
| P05 Units, extensible laws and capability checks | Prototype: strict SI sphere schema, capability/budget checks and inventory-aware density/mass/inertia compilation | Generalize dimensions, geometry, material fields and declared laws; no unsupported behavior from an LLM label |
| P06 Human/AI creator APIs | Prototype: actual Codex proposal via manual file bridge → validated preview → visible physical oak object; common human/CLI/AI recipe path | Automatic bounded model adapter, cancellation/revision feedback, second shape and resource-aware modification of existing objects |
| P07 Persistence and material history | Prototype: local inventory/provenance, recipe/allocation/object/request IDs and rigid state save/reload; replay cannot double debit | Safe revision/reclamation, migration, damage/assembly lineage; single-writer local state is not multiplayer authority or bit-exact restart |
...
| P10 Usable shared laboratory and visualization | Measured Windows workshop collection/build/run/pause/save/follow and capture; 14 suites pass using shared creator path | Automatic assistant connection and comparative shape work; detailed reference integration still needs accepted-state/budget feedback |

## Current order of work

1. Extend the working creator path: connect the assistant automatically, then add a second shape and safe revisions with resource accounting. Preserve independently validated previews, shared glass/oak/iron fixtures, one clock/contact owner and visible capability/budget limits.
...
Evidence: [creator workshop checkpoint](creator-checkpoint.md), [adaptive/12-run checkpoint](adaptive-checkpoint.md), [full-state/five-rate checkpoint](trajectory-checkpoint.md), [explicit compliance/27-case checkpoint](compliance-checkpoint.md), [contact-root/full-resolution checkpoint](event-root-checkpoint.md), [event/material comparison](event-material-checkpoint.md), [support/timestep checkpoint](coupled-support-checkpoint.md), [elastic reference](elastic-newton-checkpoint.md), [default-stage defects](material-stage-checkpoint.md), [transfer accounting](transfer-accounting-checkpoint.md), [property consumers](physics-coverage.md), [ordered roadmap](roadmap.md). The table records the scope of that evidence, including missing evidence, rather than inferring completion from passing suites.
