...
| `facing` | which way the edge faces: perpendicular to the edge, in the blade's plane, and OUT of the body -- the edge lies on a face of the body and faces away from its matter |
...
later step sweeps that matter again, so what it went through in that step is
cut as far as the edge can pay for it there -- the step's own work, and what
one equal and opposite impulse between blade and target along the way the
edge faced can still take out of the closing motion, `m_b m_t / (m_b + m_t)
v_n^2 / 2`. That impulse is sized to the declared cost of what the work did
not cover and audited by the solver, whose measured work is what is booked.
Left standing, that last stretch was a strip at the far edge of a panel
holding its halves together. Marked whatever the closing motion, it was cut
for nothing when the edge came out into something that stopped it: a cleaver
pressed through a log onto the floor bounced back up, had no closing motion
left to pay with, and a 1600 mm^2 section came apart for 623 mm^2 of work.
...
- whatever the body **held** -- fuel, water, ash, heat, a fire -- is shared
  out by the cells each piece took, through the same split a break uses
  (docs/thermochemistry.md). A burning log cut in two is two burning halves,
  each as hot as the other, holding between them the fuel the log held, and
  the energy ledger closes across the cut to its residual. A heater aimed at
  the body, or a gas region whose piston or container it was, follows the
  largest piece.

Pieces are named `<body> piece N`, like the pieces of anything else that comes
apart, and inherit the kerf that made them.

**A refused step takes the cut back.** A host can refuse a step -- the break
handshake does, when something would break -- and a refused step is taken
back whole: the bodies, every kerf's swept strips, the bonds they severed and
the joules and square metres booked are restored with the rigid state and the
chemistry, so a retried step can neither cut the same kerf twice nor burn the
same fuel twice. Measured (`tests/blade_thermo_tests.cpp`): a step refused
with a cleaver's edge 724 mm^2 into a burning log left all of that, and the
log's fuel, gas, heat and ledger, exactly as they were, and the cut then went
on from there to 1551 mm^2 for 6.98 J.
...
**The hand pushes once a step.** The push is worked out from where the body
is as the step starts and applied inside the step's reversible trial, so a
refused step takes it back with everything else. It used to be applied a
second time, by the carry that follows a held thing, on the first step after
each new aim -- and a person swinging gives a new aim every frame, so their
hand pushed with up to twice its strength. Measured now, with gravity off
and a new aim every step, the hand changes a bar's momentum by 3.327 to
3.333 kg m/s a step, against the 3.333 that 800 N is at 240 Hz. Everything
the room had measured with the doubled hand was measured again: the rope's
swing in the room is now a 60 degree turn over 0.15 s rather than one jump of
the view, and the armoury's panel is 10 mm of oak rather than 20, because a
flick with the corrected hand paid 111 J of the 20 mm board's 120.
...
- **What is cut on the way out is paid on the way out, as far as it can be.**
  In the step the edge comes out of the far side it cuts what it went through
  as far as that step's work and the closing motion can pay, and the closing
  motion's share is taken at once as one impulse along the way the edge
  faced. What neither covers stays uncut. In the armoury's strokes through
  the panel and the batten the last step's own work covered what it cut, and
  nothing was owed.
- **At 40 mm cells the section is approximate, both ways.** What is booked is
  the measured work over `R`, and at the playground yard's cells it does not
  come to the section exactly. An aluminium sword swung round a shoulder
  through 40 mm of oak 0.32 m across (12,800 mm^2) booked 13,159 mm^2 --
  59.2 J at 4.5 kJ/m^2, 2.8% over -- with a keen edge, and with a working
  edge the board came apart at 11,253 mm^2, 88% of it, when its last row of
  bonds went. In the yard's own room, a person's swing -- a 480 px drag of the
  view in about 0.2 s, in a browser -- booked 1880 mm^2 of a 1600 mm^2 rubber
  segment, 13.9 J against its 11.8 J, and the next such swing 1407 mm^2 for
  10.4 J. Where the difference comes from has not been traced; one candidate is
  that the edge is sampled every half cell and each sample in the matter
  counts as half a cell of engaged edge, so at 40 mm cells a 40 mm body is
  met by two or three samples, 40 or 60 mm. And the edge can
  go on into a piece it has just made, because a cell of that piece was
  under the steel when it was made: the swing through the yard's rope booked
  209 mm^2 (1.5 J) more in the segment's lower piece, severing no bond. At
  10 mm cells the same kind of cut books 8003 mm^2 of 8000.
...
From `tests/blade_tests.cpp` and `tests/blade_thermo_tests.cpp`, which print
every number they check, at 10 mm cells and a 1/240 s step, on the real engine
(Jolt and the lattice, nothing stubbed). Every check is against the physics,
not a number tuned until it passed.
...
| a press past R L cuts | the same blade, the same oak, an 800 N hand: through in two pieces for 4.38 J, 73% of R times the section -- separation comes when the last row of bonds goes, three quarters of the way |
...
| a burning log cut in two keeps what it held | a cleaver pressed through a burning oak log with a 300 N hand: the log was at 945.723 K with 295.667 g of fuel in 325.234 g; the halves hold 295.667 g of fuel in 325.233 g, both at 945.883 K (it went on burning while it was cut); ledger residual -2.3e-9 J before, -1.4e-9 J after, mass residual 4e-17 kg; a second later both halves burn |
| a refused step changes nothing | refused with the edge 724 mm^2 into the log: bodies, kerfs, bonds, booked work and the log's chemistry exactly as before; the cut then goes on to 1551 mm^2 for 6.98 J |
| the hand pushes once a step | gravity off, a new aim every step: the bar's momentum changes by 3.32695 to 3.33306 kg m/s a step, against 3.33333 for 800 N |
...
down it cuts nothing; a swing round a shoulder through a rope of rubber
segments, edge first, parts it and the weight falls, and the same swing flat
first cuts nothing; an edge off its body is refused), and the room itself
(`tests/world_room_tests.py`: the armoury's statics; a swing through the rope
over the same pipe the browser uses -- a 60 degree turn of the view over
0.15 s -- edge leading and flat leading; and strokes across the 10 mm panel
with the page's own hand, which part it with the upper piece still on both
fixings and the lower on the floor. One flick through the same pipe measured
6.8 m/s and 59.0 J of the board's 60).

## 11. Built by asking

The playground's chat builds with the MCP's own tools (docs/api/mcp.md), so a
cutting setup is something a person asks for, not a room laid out by hand.
The chat declares the edge with `blade`, tries the cut in its own copy of the
room with `wield` and `swing`, and reads what `cuts` says; the person's room
then opens from what it authored -- the rope whole, the sword on its rests,
the edge in the room's spec -- and the person takes the sword and swings it.

`tests/agent_build_tests.py` asks for two, in the empty yard: "Hang an iron
weight from a beam by a rope, and put a sword where I can take it, so I can
cut the rope", and the same for an oak panel on two fixings. What the chat
leaves is checked with the engine's own hand -- up off the rest, back, round
to one side, and 100 degrees round a shoulder in 0.13 s through the middle of
the rope or the panel, edge first and then flat first -- and nothing in the
check decides that anything is cut. The guide's own recipes pass it
(`--recipes`), at the yard's 40 mm cells: an aluminium sword with a working
edge, swung at 13.2 m/s, cut rope 4 in two (1059 mm^2 booked, at 7.4 kJ/m^2)
and the 4 kg weight fell 1.20 m with every other tie on; with a keen edge, at
12.2 m/s, it cut a 40 mm oak panel in two, the lower piece fell 1.58 m and
the upper stayed on both fixings; the flat of both swings cut nothing.
