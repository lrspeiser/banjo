...
**New reference checkpoint:** `1d29545035b50a32f5596274dfc9c10cded848bc` adds a coupled elastic/normal-contact CPU reference with transactional energy/momentum/gap acceptance and a real-glass convergence probe. All ten Windows CTest executables pass. [Reference results and limits](conservative-reference-checkpoint.md) show accurate accepted small-step balances but inadequate practical-step convergence/cost. This reference is not yet wired into the lab; the default-path checkpoint below and its defects remain current. Gate 1 is open; all changes remain local.
