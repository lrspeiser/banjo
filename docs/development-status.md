...
**Current trajectory checkpoint:** `e5b883e6d5d47dc85a5e1ea42c671d94a83cee99` adds complete node-state export and matched-time comparison. All 30 glass/oak/iron impact runs complete across five timesteps; all 12 CTest executables pass (8.60 s). [Measured differences](trajectory-checkpoint.md) show that bulk rebound can hide unresolved internal motion: finest-pair damped node-velocity RMS differences remain 0.0339, 0.0603 and 0.0124 m/s for glass, oak and iron. Full trajectory accuracy remains open, with a bounded state-error controller next. This probe/analysis update changes no solver law or default viewer behavior. Changes are local, not pushed or merged; Gate 1 remains open.
