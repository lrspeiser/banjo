...
**Current event/material checkpoint:** `dc49f1293dd656bac3a33e717e1a9bd29555bf5d` adds transactional event-time contact, explicit prescribed restitution/work, and glass/oak/iron comparisons without changing the catalog's material models. All 11 Windows CTest executables pass. The [36-case comparison](event-material-checkpoint.md) records accepted coarse elastic impacts alongside timestep disagreement and rejected full-resolution/dissipative cases. The [mechanics scorecard](mechanics-scorecard.md) tracks 30 mechanics and 10 platform capabilities, evidence and next steps; keep it current and retain all three materials as coverage grows. This is a local reference, not the default viewer solver. Gate 1 remains open.
