...
| scene | `"terrain": {"generate": ..., "edits": [...]}`, `"water": {"discharge_m3_s" / "rivers", "state", "watershed"}` -- the last declaring basins beyond the edges and the connections to them ([the watershed](watershed.md)) |
| line protocol | ops `dig`, `deposit` (`from_carried`: no bigger than what is carried), `cut_block`, `discharge`, `survey`, `environment`, `environment_state`, `terrain`; the ground whole when a world opens and afterwards only the rectangle that changed (`terrain_changed`); `carried` in the ground block and in every dig's and heap's reply; the water's surface and flow four times a world second; with regions beyond the edges, `beyond` in the ground block (where each connection meets its basin) and `basins` and `all_unaccounted_m3` in the water block |
| MCP | `make_terrain`, `survey`, `water_state`, `dig`, `fill` (only from what is carried), `carried` (the ground's sand and soil with what was swept up), `cut_block`, `set_river`; `add_object` sets things on the ground and says if they are in water; every `run` carries a water summary; the room's opening message says what is carried (`the_ground.carried_m3`); `make_terrain(beyond_the_edges=true)` stands a reservoir and a basin beyond the river's source and mouth, `water_state`'s `beyond_the_edges` reports them, and `set_river` feeds the reservoir |
| playground | the valley room, built by the chat from the MCP's tools; the ground and the water drawn from the engine's own heights and depths; **Dig here**, **Heap here** and the **Carried** list; a water panel; the watershed room, the same valley between a reservoir and a basin, drawn as sheets of water beyond its edges at the levels the engine reports |
