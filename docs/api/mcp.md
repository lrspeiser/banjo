...
fraction), `temperature_k`, and `rotation_deg`: how it is turned about its own
centre, in degrees -- about its own x axis, then its own y, then its own z, which
is z, then y, then x about the room's axes (`create_world`'s objects take it
too). `describe_world` gives every box that is not square to the room the
`rotation_deg` it stands at now. Every `run` carries a `heat` summary whenever
