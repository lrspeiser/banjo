...
| `fabrication_qa_run` | run the fixed native fabrication regression in isolated temporary rooms. |
| `fabrication_qa_status` | read one recorded fabrication QA report, or list runs. |
| `fabrication_qa_cancel` | cancel the owned isolated QA process and retain its report. |
...
| `define_interaction_points` | Define/read saved grip, use, surface and container receiving points; [schema and placement API](../placement-and-interaction-points.md). |
...
| `build_structure` | **build** a declared structure: the room lays it out on the ground as it is -- boards along its profile, each tilted to follow it and none over 4 m, posts from the ground under them up to what they hold, every height from a survey under its line and every side a whole number of cells -- then measures it. Nothing is left half built: a part that will not go in takes the rest out again. Measured: the chat laid the guide's flat-ground ski jump on a hillside, patched it for 30 rounds and ended "Not finished" ([Structures](#structures)) |
...
The existing fabrication QA functions also exercise the [articulation compiler](../workshop-articulation.md) and its native three-material bearing case (32 fixed cases in this checkpoint). Result rows retain measurements for constrained/free motion. This is QA coverage; `fabrication_commit` still does not accept articulated products and there is no public articulation-install tool yet.
