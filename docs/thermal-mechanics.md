...
`agent/heat-geometry` (ABI 21). Thermal expansion (D) is later. What is not done
...
| `tests/thermo_ffi_tests.py`, `tests/banjo_mcp_tests.py`, `tests/live_lanes_agree_tests.py` | the same through the C library, the MCP and both live lanes, at ABI 21 |
