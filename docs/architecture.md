...
**Starter game layer:** `StarterWorld` composes the shared recipe compiler and Jolt runtime with player XP, stamina, preset designs, tools, raw-volume stock and bounded action receipts. `banjo_starter` supplies first-person targeting and the table UI. `JoltWorld` now supports fixed boxes and removable world-fixed constraints. This is a separate starter save/rules format, not a replacement for the CreatorWorld/LLM contract. [Evidence, distinctions and integration work](starter-checkpoint.md).
...
**Coupled reference at `dc7bcfb`:** `physics/ConservativeStep` advances elastic matter and optional finite-sphere/support normal contacts in one midpoint solve, with independent balance/gap checks and transactional failure. `physics/ElasticNewton` adds global Newton/GMRES elasticity; the original local solve remains selectable. `banjo_solver_probe` measures actual-glass convergence, linear work and timing. It is not part of `RollingBallExperiment` yet; stiff sampled contact convergence, normal-impact timing, runtime cost and friction/restitution remain work. `physics/FractureStep` now adds bond failure on top of it, sharing `fracture/BondFailure.hpp` with the explicit lane and reusing the transactional rejection to re-solve an interval whose topology changed; its fragment counts are not converged under timestep refinement, see [the implicit-lane fracture checkpoint](implicit-fracture-checkpoint.md). `fracture/QuasiStaticFracture` is the static counterpart: an equilibrium solve of the live lattice under a rigid-ball load with the same shared criterion and no time integration, event-driven along the ball's travel; measured to crater rather than shatter, and inside the 1.1x realtime rule only to 2,048 cells, see [the quasi-static fracture checkpoint](fast-quasistatic-checkpoint.md). See [current global-solve evidence](elastic-newton-checkpoint.md) and the [initial coupled formulation](conservative-reference-checkpoint.md).
