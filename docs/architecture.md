...
**Event/material reference at `dc49f1293dd656bac3a33e717e1a9bd29555bf5d`:** `ConservativeAdvance` brackets new contacts using unpublished masked `ConservativeStep` trials, calls `NormalImpact` at arrivals, advances the remaining interval, and audits the entire interval before publishing. Numerical normal loss and prescribed restitution work have separate ledgers. The compiler exposes an explicit elastic approximation for glass/oak/iron without changing their declared models. See [algorithms, 36-case evidence and unresolved limits](event-material-checkpoint.md). The reference is not wired into `RollingBallExperiment`; full-resolution/dissipative event rejection and temporal accuracy remain open. Track capabilities in the [mechanics scorecard](mechanics-scorecard.md).
