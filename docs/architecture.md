...
**Coupled reference at `1d29545`:** `physics/ConservativeStep` advances elastic matter and optional finite-sphere/support normal contacts in one midpoint solve, with independent balance/gap checks and transactional failure. `banjo_solver_probe` measures actual-glass convergence. It is not part of `RollingBallExperiment` yet; practical-step convergence, contact timing, friction/restitution and damage integration remain work. See [reference evidence](conservative-reference-checkpoint.md).
