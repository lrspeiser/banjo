...
**A day for the sun: the rover rests through the night, September 22 (source, branch `agent/sun-day`):** [A day for the sun](machine-world.md#a-day-for-the-sun). The owner's next step after solar panels. A room's sun can have a day now: how long it lasts in the world's own seconds, how high the sun stands at noon, the hour the room begins at, and how strongly it shines overhead. It rises due east, stands due south at noon and sets due west, as at an equinox, on the world's own clock; low in the sky its beam is the Meinel air-mass model's share of the overhead one; below the horizon it shines on nothing, and a panel says it is night. A saved or carried world keeps the hour it had got to. The page lights the room from the sun, fades its light through twilight to a dim night, tints the sky, and the Room tab's clock says the hour. The rover needs no new program: at night its panel gives nothing, so once its battery is low it rests until morning and says so, and at dawn it says why it still rests (its panel turned away from the low sun, then in the shade of the basin's rim). In `/world?scene=tests-day` (`tools/build_rover_room.py`), a 240 s day from four in the afternoon: the sun sets 20 s in, the rover roams on in the dark, rests at 20:34 at a quarter of its 5 kJ, takes in nothing until sunrise at 06:00, and wakes at 11:00 at two fifths. A whole day's panel energy matched a fine sum of the model to a ten-thousandth. Checked by `tests/sun_day_tests.cpp`, `tests/rover_roam_tests.cpp`, `tests/day_room_tests.py` and the browser journey `ARoverRestsThroughTheNight`.

**Solar panels charge a machine's battery, and the rover rests in the sun, September 22 (source, branch `agent/solar`):** [Solar panels](machine-world.md#solar-panels). The owner's decision D1: batteries are charged by solar panels. A room can declare a sun (elevation, azimuth, irradiance; it stands still, no day yet), and a machine can carry a solar panel wired to its battery: each step it puts in the sunlight on its face -- irradiance times area times the cosine of the angle to the sun -- times its efficiency; nothing with the sun behind it or in shade, which a ray towards the sun finds and names; a full battery spills the rest; the sunlight not turned into charge is counted as heat. A battery now counts what it takes in, and what it holds is what it began with, plus that, less what it gave -- to the joule in the tests. The roam program can rest: below a share of full it stops where it is on its brakes until the sun has charged it back to another. In `/world?scene=tests-solar` (`tools/build_rover_room.py`) the rover with a 5 kJ battery at 28% roams, rests at a quarter while its 0.2 m2 glass panel gives 29.8 W of 148.8 W of sunlight, and roams on at three fifths; the page is lit from the engine's sun and the Machines list says what the panel gives. The rover's caster wheel is iron now: an oak one, light under the rover's weight, sank 27 mm into the ground at 1/120 s steps and the rover stuck. Checked by `tests/solar_panel_tests.cpp`, `tests/rover_roam_tests.cpp`, `tests/solar_room_tests.py` and the browser journey `ARoverRestsInTheSun`. Not built: a day, when the sun moves and sets.

**A rover that roams a lake's shore by itself, September 22 (source, branch `agent/rover-roam`):** [Milestone 2, its second step](machine-world.md#milestone-2-its-second-step-a-rover-that-roams). A machine can have a program now (`LiveProgram`): a layer above its controllers that works them the way a person works their panels, from what its sensors and its own slope read, turned on and off from its own panel. The one kind, "roam", drives a rover with a motor on each back wheel and a caster in front: forward; from water a front sensor sees, back off and turn away from that side; from ground steeper than 8 degrees, ahead or to the side, turn downhill; from wheels that stall, back off and turn. Nothing tells it where the lake is. In `/world?scene=tests-rover` (`tools/build_rover_room.py`, which lets it roam a minute in the engine and refuses to write the room unless it stayed dry and in the basin) it roamed 50.3 m in a minute, turned away 5 times and never had a wheel in water; saved while roaming, it comes back doing the same thing. Three designs failed that check first: turning on the spot where it saw the water swung the caster in; sensors inside the wheels' track, looking for a centimetre, let a back wheel run into water they had not seen; and a slope check ahead only let it spiral up a contour. A controller's parts now follow pins on through every part that moves with the machine, so E on the caster's wheel finds the rover. Checked by `tests/rover_roam_tests.cpp`, `tests/rover_room_tests.py` and the browser journey `ARoverRoamsTheShore`. Not built: a bump sensor that feels a knock, and a charging post it finds and docks at, which waits on decision D1.
