...
The first shipped runtime slice may add a `SparseThermalWorld` and pure `ThermalKernel` while keeping mechanics unchanged. That slice is deliberately narrow: fixed-capacity/conductivity regions, finite fuel/oxygen lumped reaction, enthalpy with latent heat, bounded jobs, explicit lag, and no cross-region or cold-region flux, mechanics coupling, fluid flow, or automatic activation. One thermal region may span multiple storage chunks, but it is still a single insulated solve. It is a thermal foundation, not a full fire system or a mechanics certification.
