...
| `POST /api/world/open` | Open the room on /world: `{scene}` (one of the rooms above) or `{qa: "<run>/<case>-<trial>"}` (a saved QA build). A page opening the room this server is already running (a reload, or a second tab) rejoins it as it stands — every body where it is and as it is, moved, broken or dented, and the hand still holding what it held — and the reply says `rejoined: true`; the page that had the room is refused from then on. The first open after the server has restarted opens the room into the world it saved (`world` in the room's file, `banjo.room.v2`): every body where it was and as it was, moved, broken, dented or cut, joints at their angles, the bag's things still set aside and the hand still holding what it held. The reply then says `kept: true` and `restored: {tier: "whole", saved_t_s, bodies, not_kept, parked}`; `not_kept` lists, in words, what a restart does not give back yet (heat, char and fuel are declared again from the room; anything that was under way). The world is saved after each accepted inventory change, after a break is worked out, every 5 s of the world's time while the page steps it, after the ground changes, after the room opens, and as the server stops. A saved world is used only for the spec it was saved from, so one saved before the chat changed the room is dropped. One the engine will not put back whole is set aside beside the room (`<room>.world-set-aside-<time>.json`, never deleted), and the room opens from what it is held as, with whole things put back where they were left when it can; the reply says why in `kept_problem`. The in-process lane (`--live-inprocess`) restores through the C library (`banjo_open_snapshot`). `again: true` (the page's "Start the room again") opens it again from what it is held as; `fresh: true` builds it again from scratch |
| `POST /api/world/ask` | One chat turn in the open room: `{session, message, story, person}` — `session` is the room the page has open (from `/api/world/open`), and a page whose room was opened again elsewhere is refused, with no model asked; `person` is where you are (`standing_m`, `eyes_m`, `facing`, `looking_at`, `looking_at_m`), checked and passed to the chat as `the_person`; a room the chat changed is opened again from what it left, carrying the world that was running (`server.world_to_carry`), so everything the change did not touch is as it stood, and the reply's `state.restored` (tier `carried`) says what did not come back as it was |
