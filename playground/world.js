...
async function tick() {
  ...
  world.busy = true;
  // Which world this step is for. Switching rooms opens a new one while a step
  // for the old one can still be in flight, and that step comes back "that live
  // world is no longer open" -- which is true of the OLD world only.
  const stepping = world.session;
  try {
    ...
    // Only the world that failed stops. Taking the old room's answer as the new
    // room's failure cleared the new session before it had drawn a frame, and
    // every request after that -- the chat included -- was silently dropped.
    if (world.session === stepping) {
